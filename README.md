...
- 과제 내용
  - 아이디 또는 패스워드가 잘못 입력되었을 때 발생하는 에러를 MessageBox를 띄우는 대신 화면에 직접 표시하도록 기능을 개선합니다.
  - Label 컨트롤을 새롭게 추가하고 Visible 속성을 제어하여 상황에 따라 에러 메시지를 보이고 숨기는 기능을 구현합니다.

- 구현 내용과 기능 설명
  - 기존 로그인 화면 하단에 에러를 안내할 Label 컨트롤(lblErrorMsg)을 추가로 배치하고, 프로그램 시작 시에는 Visible = false 속성을 주어 눈에 보이지 않게 초기화했습니다.
  - 로그인 버튼 클릭 시 if~else 조건문을 거쳐 로그인에 실패했을 때, 기존의 MessageBox 대신 lblErrorMsg.Visible = true; 코드가 실행되도록 수정했습니다. 이를 통해 화면에 직관적인 빨간색 에러 문구가 나타나도록 사용자 경험(UX)을 개선했습니다.
  - 이전에 로그인에 실패하여 에러 메시지가 떠 있는 상태에서, 다시 올바른 계정 정보를 입력해 로그인에 성공한다면 lblErrorMsg.Visible = false;가 작동하여 떠 있던 에러 메시지를 다시 숨김 처리하도록 세부 로직을 보완했습니다.